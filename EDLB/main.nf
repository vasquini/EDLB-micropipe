...
process BASECALL_DORADO {
	...
	${params.dorado_gpu_folder}/dorado basecaller -b ${params.dorado_batchsize} -r -x ${params.dorado_device} ${params.dorado_model} ${reads}  > ${sample_id}.bam
	"""
}
...
process flye {
	...
	flye --nano-hq ${filtered} --genome-size ${genome_size} --threads ${params.flye_threads} --out-dir \$PWD ${params.flye_args}
